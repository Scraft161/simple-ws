...
pub fn convert_wiki(path: &str) -> Result<html_node::Node, Box<dyn Error>> {
    ...
    let mdast = markdown::to_mdast(&md, &markdown::ParseOptions {
                constructs: markdown::Constructs {
                    //character_reference: true,
                    frontmatter: true,
                    html_flow: true,
...
fn traverse_mdast(node: markdown::mdast::Node, ignore_p: bool) -> Option<html_node::Node> {
    match node {
        mdast::Node::Root(root) => {
            ...
            // Generate Index
            let index = generate_index(html_node::Fragment{children: children.clone()});
        ...
        mdast::Node::Heading(heading) => {
            ...
            let html_node::Node::Text(text) = &children[0] else { return None };
        ...
        mdast::Node::InlineCode(code) => Some(html!(<code class="inline-code">{text!("{}", code.value)}</code>)),
...
fn id_from_text(text: &str) -> (String, Option<usize>) {
    if HEADING_WITH_ID.is_match(text) {
        ...
        let Some((_, id)) = re_match.as_str().split_once('{') else {todo!()};
        let Some((id, _)) = id.rsplit_once('}') else {todo!()};
            ...
            if let html_node::Node::Element(element) = node {
                match element.name.as_str() {
                    ...
                    "h4" => {
                        ...
                        let (id, _pos) = id_from_text(&name);
                        index.sub_headings[h2_len - 1].sub_headings[h3_len - 1].sub_headings.push(H4 {
                            name,
                            id,
                        })
                    },
	...
	fn from_yaml(yaml: &str) -> Self {
		...
		for str in yaml.split("\n") {
			if let Some(yaml_title) = str.strip_prefix("title: ") {
				...
				yaml_tags.split(',').for_each(|s| tags.push(FTag::from_str(s.trim()).unwrap()))
			}
		}
